...
int main(int argc, char* argv[]){
  ...
  if(TimeCutFile!=NULL){
    TFile* tcFile = new TFile(TimeCutFile);
    delayed = (TCutG*)tcFile->Get("delayed");
    prompt = (TCutG*)tcFile->Get("prompt");
  }
  ...
  TH2F* pid = new TH2F("pid","pid",500,2.5,3.0,500,17,28);hlist->Add(pid);
  ...
  vector<TH1F*> en_prompt;
  vector<TH1F*> en_delayed;
  vector<TH1F*> en_before;
  en_vs_ti_big.resize(PidCut.size()+1);
  en_vs_ti.resize(PidCut.size()+1);
  en_vs_ti_small.resize(PidCut.size()+1);
  en_prompt.resize(PidCut.size()+1);
  en_delayed.resize(PidCut.size()+1);
  en_before.resize(PidCut.size()+1);
  ...
  for(unsigned short i=0; i<PidCut.size();i++){
    ...
    en_prompt[i] = new TH1F(Form("en_prompt_%s",PidCut[i]->GetName()),Form("en_prompt_%s",PidCut[i]->GetName()),4000,0,2000);hlist->Add(en_prompt[i]);
    en_delayed[i] = new TH1F(Form("en_delayed_%s",PidCut[i]->GetName()),Form("en_delayed_%s",PidCut[i]->GetName()),4000,0,2000);hlist->Add(en_delayed[i]);
    en_before[i] = new TH1F(Form("en_before_%s",PidCut[i]->GetName()),Form("en_before_%s",PidCut[i]->GetName()),4000,0,2000);hlist->Add(en_before[i]);
  }
  ...
  en_prompt[PidCut.size()] = new TH1F("en_prompt_all","en_prompt_all",4000,0,2000);hlist->Add(en_prompt[PidCut.size()]);
  en_delayed[PidCut.size()] = new TH1F("en_delayed_all","en_delayed_all",4000,0,2000);hlist->Add(en_delayed[PidCut.size()]);
  en_before[PidCut.size()] = new TH1F("en_before_all","en_before_all",4000,0,2000);hlist->Add(en_before[PidCut.size()]);
  ...
  for(int i=0; i<nentries;i++){
    ...
    for(unsigned short j=0;j<eurica->GetHits().size();j++){
      ...
      if(prompt&&prompt->IsInside(hit->GetTime(),hit->GetEnergy()))
	en_prompt[PidCut.size()]->Fill(hit->GetEnergy());
      if(delayed&&delayed->IsInside(hit->GetTime(),hit->GetEnergy()))
	en_delayed[PidCut.size()]->Fill(hit->GetEnergy());
      if(hit->GetTime()>420)
	en_before[PidCut.size()]->Fill(hit->GetEnergy());
	  ...
	  if(prompt&&prompt->IsInside(hit->GetTime(),hit->GetEnergy()))
	    en_prompt[k]->Fill(hit->GetEnergy());
	  if(delayed&&delayed->IsInside(hit->GetTime(),hit->GetEnergy()))
	    en_delayed[k]->Fill(hit->GetEnergy());
	  if(hit->GetTime()>420)
	    en_before[k]->Fill(hit->GetEnergy());
    	}
      }
    }
